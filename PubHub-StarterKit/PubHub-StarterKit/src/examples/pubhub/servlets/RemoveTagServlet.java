...
public class RemoveTagServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (tempBook != null) {
			//ASSERT: the book exist to remove the tag
			...
			if (doesExist) {
				...
				boolean isSuccess = database2.removeTag(tag);
				...
				doesExist = false;
				req.getSession().setAttribute("message", "Tag of " + nameTag + " does not exist for given isbn");
				req.getSession().setAttribute("messageClass", "alert-danger");
				req.getRequestDispatcher("tagDetails.jsp").forward(req, resp);
			}
		} else {
